...
def select_best_mentor(mentors_scores, current_depth):
    ...
    """
    # If at the first step, randomly select a mentor to encourage exploration and gather initial performance data for all mentors.
    if current_depth == 0:
    ...
    competitiveness_scores = {} # dictionary to store the competitiveness score for each mentor, which is calculated as the product of their past scores. This reflects both the performance and consistency of the mentor across all previous steps. A higher score indicates better and more consistent performance.
    for mentor, scores in mentors_scores.items():
        competitiveness_scores[mentor] = 1.0 # Start with a score of 1 for multiplication
        
        for score in scores:
            competitiveness_scores[mentor] *= score # e.g. if a mentor has scores [0.2, 0.2, 1], the competitiveness score will be 0.04, indicating that the mentor's performance is not consistently good across steps. A mentor with scores [1, 1, 1] will have a competitiveness score of 1, indicating strong and consistent performance.

    # Find the maximum competitiveness score
    # competitiveness_scores: {'mentor1': 0.04, 'mentor2': 1.0, 'mentor3': 0.008}
    # .item returns a list of (mentor, score) pairs, and max() finds the pair with the highest score. In this example, it would return ('mentor2', 1.0).
    # .values returns a view of the scores, and max() finds the highest score among them. In this example, it would return 1.0.
...
def select_next_step(steps, previous_mentors=None):
    ...
    top_steps = [s for s in steps if s.score == max_score] # take the steps with the highest score, which may be multiple if there's a tie. We will apply tie-breaking logic to select among them.
    
    # If there's only one highest scoring step, return it directly
    if len(top_steps) == 1:
        return top_steps[0]
    
    # take the mentors who generated the top steps, and if there are mentors that have not been selected before
    # we prioritize those to encourage diversity in mentor selection and avoid getting stuck with the same mentor in case of ties. 
    # This allows us to explore different reasoning paths and gather more information about the performance of all mentors.
    new_mentors_steps = [s for s in top_steps if s.generated_by not in previous_mentors]

    if new_mentors_steps:
        return random.choice(new_mentors_steps) # randomly select among the top steps generated by mentors that have not been selected before to encourage diversity in mentor selection and avoid getting stuck with the same mentor in case of ties. This allows us to explore different reasoning paths and gather more information about the performance of all mentors.

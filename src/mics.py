        ...
        for depth in range(self.args.max_depth):
            ...
            for mentor_model in self.mentor_models:
                ...
                if current_step.parent is None:
                    ...
                    # for root, we extract the first two steps to speed up the search
                    ...
                    if current_step.generated_by == mentor_model:
                        try:
                            # don't call the same mentor again, 
                            # just extract the next step from the cached reasoning chain for this mentor. 
                            # This is important for efficiency, especially at deeper layers where we want to avoid redundant calls to the same mentor if it continues along the path. We can simply extract the next step from the cached reasoning chain for this mentor, which was generated when this mentor was first selected at a previous depth.
